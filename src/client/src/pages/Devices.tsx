...
import {
  GET_DEVICES_BY_MODELID_WITH_PAGINATION,
  GET_DEVICES_TOTAL_BY_MODELID,
...
function Devices() {
  ...
  const [getDevices, { error, loading, data: devPag }] = useLazyQuery(
    GET_DEVICES_BY_MODELID_WITH_PAGINATION
  );
  useEffect(() => {
    getDevices({
  ...
  if (devPag) {
    ...
    if (store.selection && store.selection === "all") {
      ...
      result = devPag.getDevicesByModelIdWithPagination.filter(
        (i: { userId: null | string }) => i.userId === null
      );
    } else if (store.selection && store.selection === "outOfStock") {
      result = devPag.getDevicesByModelIdWithPagination.filter(
        (i: { userId: null | string }) =>
          i.userId !== currentUserId && i.userId !== null
      );
    } else if (store.selection && store.selection === "withMe") {
      result = devPag.getDevicesByModelIdWithPagination.filter(
        (i: { userId: null | string }) => i.userId === currentUserId
      );
    }
  }
  ...
  if (error) {
    return (
      <div className="loading">
        <h1 className="loading__text">{error.message}</h1>
      </div>
    );
  }
          ...
          <ListCards>
            {result.map((device: ModelDeviceData) => (
              <StyledDeviceCard
                key={device.id}
                availability={
                  (device.userId === null && "In Stock") ||
                  (device.userId === currentUserId ? "With Me" : "Out Of Stock")
                }
                deviceId={device.id}
                backgroundcolor={
                  (device.userId === null && "#5AB946") ||
                  (device.userId === currentUserId ? "#00A5D9" : "#ED1534")
                }
                onClick={() => {
