...
// import { GET_ALL_DEVICES, GET_ALL_STOCK_DEVICES } from "../graphql/devices";
import { GET_ALL_STOCK_DEVICES } from "../graphql/devices";
...
const DashboardStockDevices = () => {

  const { error, loading, data } = useQuery(GET_ALL_STOCK_DEVICES);
  if (data) {
    console.log(data);
  }
  ...
  return (
    <AdminLayout>
      <Title>Devices in stock</Title>

      {loading && (<p>Loading ...</p>)}
      {error && (<p>{error.message}</p>)}
      {data && <Table  data={data.stockDevices} columns={columns} />}
...
export default DashboardStockDevices;
