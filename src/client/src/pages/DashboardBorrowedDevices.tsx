...
const columns: GridColDef[] = [
  ...
  { 
    ...
    renderCell: (params) => {
      return (
        <span>{params.row.user.firstName} {params.row.user.lastName}</span>
      )
    }
  },
  { 
...
const DashboardBorrowedDevices = () => {

  const { error, loading, data } = useQuery(GET_ALL_BORROWED_DEVICES);
  if (data) {
    console.log(data);
  }
  ...
  return (
    <AdminLayout>
      <Title>Borrowed devices</Title>

      {loading && (<p>Loading ...</p>)}
      {error && (<p>{error.message}</p>)}
      {data && <Table  data={data.borrowedDevices} columns={columns} />}
...
export default DashboardBorrowedDevices;
