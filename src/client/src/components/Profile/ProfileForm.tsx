...
const ProfileForm : React.FC = () => {
  ...
  const formik: FormikProps<EditProfileValues> = useFormik<EditProfileValues>({
    ...
    validationSchema: YUP.object({
      ...
      editEmail: YUP.string().email("Inavalid email address ").required("Email is required"),
     
      editPass: YUP.string()
      .min(6, "Password must contain between 6 and 12 characters ")
      .max(12, "Password must contain between 6 and 12 characters")
      .required("Password is required"),
      repeatEditPass: YUP.string().oneOf([YUP.ref('editPass'), null], 'Passwords must match'),
    ...
    <EditProfileSection>
      <div className="wrap">
        ...
        <form onSubmit={formik.handleSubmit}>
          ...
          />
          {formik.touched.repeatEditPass && formik.errors.repeatEditPass ? <p className="error">{formik.errors.repeatEditPass}</p> : null}
          <StyledButton 
            type="submit" 
            text="Update Account" 
            name="updateAccount"
