...
export class ReservationTimesService {
  constructor(
    ...
    private reservationTimesRepository: Repository<ReservationTime>,
  ) {}

  create(
    createReservationTimeInput: CreateReservationTimeInput,
  ): Promise<ReservationTime> {
    const newReservationTime = this.reservationTimesRepository.create(
      createReservationTimeInput,
    );
  ...
  async update(
    ...
    const updatedReservationTime =
      await this.reservationTimesRepository.preload({
        id: id,
